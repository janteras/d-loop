...
const fromWei = (amount) => ethers.formatEther(amount);
...
describe("Critical Functions Test Suite", function() {
  ...
  before(async function() {
    ...
    aiNodeRegistry = await AINodeRegistry.deploy(await dloopToken.getAddress(), await soulboundNFT.getAddress());
    ...
    treasury = await Treasury.deploy(await dloopToken.getAddress());
    ...
    await aiNodeRegistry.setGovernanceContract(await aiNodeGovernance.getAddress());
    await dloopToken.mint(await treasury.getAddress(), toWei(1000000)); // Mint 1M tokens to treasury
