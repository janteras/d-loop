...
// Permission levels (optimized version with better access control)
...
function formatMethodAnalysis(contract, method) {
  ...
  const approvalColor = approvalStatus === 'Public' ? '\x1b[33m' : '\x1b[32m';
...
function calculatePrivilegeEscalationRisk(contractName) {
  ...
  let restrictedMethodCount = 0;
  
  for (const [role, methods] of Object.entries(permissions)) {
    if (role === 'public') {
      publicMethodCount += methods.length;
    } else if (role === 'restricted') {
      restrictedMethodCount += methods.length;
    } else {
  ...
  const totalMethods = adminMethodCount + publicMethodCount + restrictedMethodCount;
  const riskRatio = (adminMethodCount + restrictedMethodCount) / totalMethods;
...
async function generateGasProfileSummary() {
  ...
  const privilegeEscalationRisk = (accessControlledMethods / totalMethods) >= 0.85 ? "LOW" : "POTENTIAL CONCERN";
