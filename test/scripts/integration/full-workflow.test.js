...
describe("D-Loop Protocol Integration", function () {
  ...
  beforeEach(async function () {
    ...
    await dloopToken.transfer(governanceRewards.address, ethers.utils.parseEther("100000"));
  ...
  describe("End-to-End Protocol Flow", function () {
    ...
    it("Should set up AI node governance with delegations", async function () {
      ...
      expect(user1Delegation.amount).to.equal(ethers.utils.parseEther("20000"));
      expect(user2Delegation.amount).to.equal(ethers.utils.parseEther("15000"));
      ...
      expect(node1Power).to.equal(ethers.utils.parseEther("50000")); // 30K stake + 20K delegated
      expect(node2Power).to.equal(ethers.utils.parseEther("40000")); // 25K stake + 15K delegated
    ...
    it("Should execute full investment and divestment workflow", async function () {
      ...
      expect(wethDetails.balance).to.equal(ethers.utils.parseEther("5"));
      expect(linkDetails.balance).to.equal(ethers.utils.parseEther("50"));
      ...
      expect(user1DAIBalance).to.be.gt(ethers.utils.parseEther("9900"));
      expect(user2DAIBalance).to.be.gt(ethers.utils.parseEther("990"));
      ...
      expect(finalUser1WETH).to.be.gt(initialUser1WETH.add(ethers.utils.parseEther("1.98"))); // ~2 WETH
      expect(finalUser2LINK).to.be.gt(initialUser2LINK.add(ethers.utils.parseEther("24.5"))); // ~25 LINK
      ...
      expect(finalWethDetails.balance).to.equal(ethers.utils.parseEther("3")); // 5 - 2
      expect(finalLinkDetails.balance).to.equal(ethers.utils.parseEther("25")); // 50 - 25
  ...
  describe("Error and Recovery Scenarios", function () {
    it("Should handle failed proposals correctly", async function () {
      ...
      await assetDAO.connect(user1).createProposal(
        0, // ProposalType.Investment
        weth.address,
        ethers.utils.parseEther("50"), // Very large amount
