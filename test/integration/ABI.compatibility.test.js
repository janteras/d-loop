...
describe("ABI Compatibility Verification", function() {
  ...
  before(async function() {
    ...
    // Get accounts - handle both ethers v5 and v6
    let accounts;
    let signers;
    
    try {
      // ethers v6 approach
      if (typeof provider.getWallets === 'function') {
        // Hardhat-specific method
        signers = await provider.getWallets();
      } else if (typeof provider.getSigner === 'function') {
        // ethers v5 approach
        owner = provider.getSigner(0);
        admin = provider.getSigner(1);
        user = provider.getSigner(2);
      } else {
        // ethers v6 approach without built-in wallet management
        ...
        const testAccounts = [
          ...
          '0x5de4111afa1a4b94908f83103eb1f1706367c2e68ca870fc3fb9a804cdab365a'  // Third Hardhat account
        ];
        
        owner = new ethers.Wallet(testAccounts[0], provider);
        admin = new ethers.Wallet(testAccounts[1], provider);
        user = new ethers.Wallet(testAccounts[2], provider);
      }
      
      // Verify we have the signers
      console.log(`Owner address: ${await getAddress(owner)}`);
      console.log(`Admin address: ${await getAddress(admin)}`);
      console.log(`User address: ${await getAddress(user)}`);
    } catch (error) {
      console.error(`Error setting up signers: ${error.message}`);
      throw error;
    }
    ...
    try {
      ...
      // Get addresses using our helper function
      ...
      console.log(`SoulboundNFT deployed at: ${await getAddress(soulboundNFT)}`);
      ...
      console.log(`AINodeRegistry deployed at: ${await getAddress(aiNodeRegistry)}`);
      ...
      await soulboundNFT.connect(admin).grantRole(MINTER_ROLE, await getAddress(aiNodeRegistry));
  ...
  describe("Cross-Contract Interactions", function() {
    it("should verify SoulboundNFT roles apply correctly in integration context", async function() {
      ...
      const aiNodeRegistryAddress = await getAddress(aiNodeRegistry);
      ...
      const anotherUserAddress = await getAddress(anotherUser);
