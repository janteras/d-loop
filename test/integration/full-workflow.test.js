...
describe("D-Loop Protocol Integration", function () {
  ...
  const FEE_MANAGER_ROLE = keccak256(toUtf8Bytes("FEE_MANAGER_ROLE"));
  const AUTHORIZED_CONTRACT_ROLE = keccak256(toUtf8Bytes("AUTHORIZED_CONTRACT_ROLE"));
  const DISTRIBUTOR_ROLE = keccak256(toUtf8Bytes("DISTRIBUTOR_ROLE"));
  const FEE_PROCESSOR_ROLE = keccak256(toUtf8Bytes("FEE_PROCESSOR_ROLE"));
  ...
  beforeEach(async function () {
    ...
    await dloopToken.transfer(user1.address, parseEther("50000"));
    await dloopToken.transfer(user2.address, parseEther("50000"));
    await dloopToken.transfer(user3.address, parseEther("10000"));
    await dloopToken.transfer(aiNode1.address, parseEther("50000"));
    await dloopToken.transfer(aiNode2.address, parseEther("50000"));
    ...
    await priceOracle.setDirectPrice(
      weth.address,
      ethers.utils.parseUnits("2000", 8), // $2000 per ETH
      8
    );
    
    await priceOracle.setDirectPrice(
      link.address,
      ethers.utils.parseUnits("20", 8), // $20 per LINK
      8
    );
    
    await priceOracle.setDirectPrice(
      usdc.address,
      ethers.utils.parseUnits("1", 8), // $1 per USDC
      8
    );
    ...
    await dloopToken.transfer(governanceRewards.address, parseEther("100000"));
    ...
    await assetDAO.updateGovernanceParameters(
      ...
      parseEther("1000") // 1,000 DLOOP min stake
    );
  ...
  describe("End-to-End Protocol Flow", function () {
    ...
    it("Should set up AI node governance with delegations", async function () {
      ...
      expect(user1Delegation.amount).to.equal(parseEther("20000"));
      expect(user2Delegation.amount).to.equal(parseEther("15000"));
      ...
      expect(node1Power).to.equal(parseEther("50000")); // 30K stake + 20K delegated
      expect(node2Power).to.equal(parseEther("40000")); // 25K stake + 15K delegated
    ...
    it("Should execute full investment and divestment workflow", async function () {
      ...
      expect(wethDetails.balance).to.equal(parseEther("5"));
      expect(linkDetails.balance).to.equal(parseEther("50"));
      ...
      expect(user1DAIBalance).to.be.gt(parseEther("9900"));
      expect(user2DAIBalance).to.be.gt(parseEther("990"));
      ...
      expect(finalUser1WETH).to.be.gt(initialUser1WETH.add(parseEther("1.98"))); // ~2 WETH
      expect(finalUser2LINK).to.be.gt(initialUser2LINK.add(parseEther("24.5"))); // ~25 LINK
      ...
      expect(finalWethDetails.balance).to.equal(parseEther("3")); // 5 - 2
      expect(finalLinkDetails.balance).to.equal(parseEther("25")); // 50 - 25
    ...
    it("Should execute protocol governance workflow", async function () {
      ...
      await protocolDAO.connect(aiNode1).createProposal(
        ...
        protocolDAO.interface.encodeFunctionData("updateGovernanceParameters", [
          ...
          parseEther("2000"), // 2000 DLOOP min stake
      ...
      expect(await protocolDAO.minProposalStake()).to.equal(parseEther("2000"));
      ...
      expect(finalAINodeBalance).to.be.gt(parseEther("50000"));
  ...
  describe("Error and Recovery Scenarios", function () {
    it("Should handle failed proposals correctly", async function () {
      ...
      await assetDAO.connect(user1).createProposal(
        0, // ProposalType.Investment
        weth.address,
        parseEther("50"), // Very large amount
