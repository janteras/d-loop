...
async function main() {
  try {
    ...
    execSync('npx hardhat compile --config hardhat.config.simple.js');
...
// Run the tests
console.log('Starting Fixed FeeCalculator Test');
main().catch(error => {
  console.error('Test failed:', error);
  shutdownHardhatNode();
  process.exit(1);
