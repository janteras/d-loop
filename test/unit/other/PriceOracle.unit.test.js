...
require("../../utils/ethers-v6-shim.js");

describe("PriceOracle", function() {
  // ... existing PriceOracle tests ...
});

// ChainlinkPriceOracle tests appended after all existing tests
describe("ChainlinkPriceOracle", function () {
  let owner, fallbackAdmin, user;
  ...
  beforeEach(async function () {
    [owner, fallbackAdmin, user] = await ethers.getSigners();
    ...
    await chainlinkOracle.connect(fallbackAdmin).setFeed(
      mockToken.address,
  ...
  it("falls back to fallback price if aggregator not set", async function () {
    ...
    await chainlinkOracle.connect(fallbackAdmin).setFeed(mockToken.address, ethers.constants.AddressZero, 3600);
  ...
  beforeEach(async function() {
    ...
    priceOracle = await PriceOracle.deploy(admin.address, priceUpdater.address);
